...
- 运行完成后，在该工作流的 `Artifacts` 下载两个文件：
  - `chat8-backend_amd64.tar.gz`
  - `chat8-frontend_amd64.tar.gz`
- 在目标机器导入（Linux/WSL/Windows PowerShell 皆可，路径按实际替换）：
  - `docker load -i /path/to/chat8-backend_amd64.tar.gz`
  - `docker load -i /path/to/chat8-frontend_amd64.tar.gz`
- 使用生产编排启动（如需同域部署）：
...
**适用对象**：你的朋友或运维在一台新的 Ubuntu 22.04/24.04 服务器，或在 Windows 上用 WSL2 的 Ubuntu 子系统，希望“一次下载、一次运行”。
...
**二、两种部署方式（任选其一）**
- 方式 A：使用离线应用镜像（最稳，不依赖 npm/pip/Docker Hub）
  - 从 GitHub Actions 的 `Artifacts` 下载：
    - `chat8-backend_amd64.tar.gz`
    - `chat8-frontend_amd64.tar.gz`
  - 上传到服务器并导入（示例）：
    - `docker load -i ~/chat8-backend_amd64.tar.gz`
    - `docker load -i ~/chat8-frontend_amd64.tar.gz`
  - 启动（同域反代、免 CORS）：
  ...
  - 访问与验证：
    - 前端：`http://<服务器IP>:8080`
    - 后端：`http://<服务器IP>:8000`
    - 日志：`docker logs -n 100 chat8-frontend`、`docker logs -n 100 chat8-backend`
...
- 性能建议：将代码复制到 WSL 文件系统（如 `~/work/chat8`）再运行容器。