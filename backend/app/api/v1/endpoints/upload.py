...
async def upload_file_internal(
    ...
    encrypted: bool,
    method: str,
    destroy_after: Optional[int],
    hidding_message: Optional[str],
    ...
    try:
        ...
        if not content:
            if file_type == "image":
                auto_content = f"发送了图片: {file.filename}"
            else:
                auto_content = f"发送了文件: {file.filename}"
        else:
        ...
        message = message_service.send_message(
            ...
            message_type=file_type,  # 使用传入的文件类型
